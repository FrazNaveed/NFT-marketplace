...
contract NFT is ERC721 {
    ...
    event Log(address, uint256, bytes data);
    ...
    fallback () external payable { emit Log(msg.sender, msg.value, msg.data);}
    ...
    function mint(
        ...
        uint256 price
    ) public returns (uint256 mintedId) {

        _mint(to, tokenId);
        mintedIdsArray.push(tokenId);
    ...
    function buyToken(uint256 tokenId) payable public {
       ...
       // uint256 price = getTokenPrice(tokenId);
        ...
        (bool sent, bytes memory data) = oldOwner.call{value: msg.value}("");
    ...
    function getLastMint() external view returns (uint256 lastMint) {
        lastMint = _lastMint;
    }
    ...
    function getLastNTokens(uint256 n)
        external
        view
    ...
    function getCollections(address to) public view returns (uint256[] memory) {
        return collections[to];
    }
}